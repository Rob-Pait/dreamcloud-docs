...
These instructions were performed with **s3cmd v1.5.2**. If you'd like to install a different version, you'll need to modify the file names appropriately.
...
3. Download the `latest release of s3cmd from GitHub <https://github.com/s3tools/s3cmd/releases>`_.
...
6. Copy the s3cmd executable and S3 folder into the bin directory we created earlier
...
7. Add the bin directory to your path so that you can execute the newly installed script.
    ...
    .. note:: This assumes you're using the default bash shell. If you're using a different shell, you'll have to:: set the path in the proper place.
...
3. Include your access key and secret key from the `DreamObjects control panel <https://panel.dreamhost.com/index.cgi?tree=cloud.objects&>`_.
